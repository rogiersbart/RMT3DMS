...
read_dsp <- function(file, btn)
{
  ...
  # Data set C1
    dataSetC1 <- int_get_mt3dms_array(dsp.lines,btn$NROW,btn$NCOL,btn$NLAY)
    dsp$AL <- dataSetC1$mfarray
    dsp.lines <- dataSetC1$remaining.lines
    rm(dataSetC1)
 
  # Data set C2
    dataSetC2 <- int_get_mt3dms_array(dsp.lines,1,btn$NLAY,1)
    dsp$TRPT <- dataSetC2$mfarray
    dsp.lines <- dataSetC2$remaining.lines
    rm(dataSetC2)
  
  # Data set C3
    dataSetC3 <- int_get_mt3dms_array(dsp.lines,1,btn$NLAY,1)
  ...
  # Data set C4
    if(dsp$MultiDiffusion)
    {
      ...
      {
        dataSetC4 <- int_get_mt3dms_array(dsp.lines,btn$NROW,btn$NCOL,btn$NLAY)
        dsp$DMCOEF[[comp]] <- dataSetC4$mfarray
        dsp.lines <- dataSetC4$remaining.lines
      }
      rm(dataSetC4)
    } else {
      dataSetC4 <- int_get_mt3dms_array(dsp.lines,1,btn$LAY,1)
