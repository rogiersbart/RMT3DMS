...
get_mt3dms_array <- function(mfarray.lines,NROW,NCOL,NLAY)
{
  ...
  if(prod(dim(mfarray))!=0)
  {
    ...
    { 
      ...
      if(remove.empty.strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='0') 
      {
        if(NLAY==1)
        {
          mfarray[,,] <- rep(as.numeric(remove.empty.strings(strsplit(mfarray.lines[1],' |\t')[[1]])[2]),NROW*NCOL*NLAY)
      ...
      else if(remove.empty.strings(strsplit(mfarray.lines[1],' ')[[1]])[1] %in% c('100','103'))
      {
        mfarray.lines <- mfarray.lines[-1] 
        ...
        mfarray[,,k] <- matrix(as.numeric(remove.empty.strings(strsplit0(paste(mfarray.lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]])),nrow=NROW,ncol=NCOL,byrow=TRUE)
