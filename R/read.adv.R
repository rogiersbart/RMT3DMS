...
#' @export
read.adv <- function(file)
{
  adv.lines <- scan(file, what=character(), sep='\n')
  adv <- NULL
 
  # Data set B1
    dataSetB1 <- remove.empty.strings(strsplit(adv.lines[1],' ')[[1]])
  ...
  # Data set B2
    ...
    {  
      dataSetB2 <- remove.empty.strings(strsplit(adv.lines[1],' ')[[1]])
      adv.lines <- adv.lines[-1]  
  ...
  # Data set B3
    ...
    {  
      dataSetB3 <- remove.empty.strings(strsplit(adv.lines[1],' ')[[1]])
      adv.lines <- adv.lines[-1]  
  ...
  # Data set B4
    ...
    {  
      dataSetB4 <- remove.empty.strings(strsplit(adv.lines[1],' ')[[1]])
      adv.lines <- adv.lines[-1]  
  ...
  # Data set B5
    if(adv$MIXELM==3)
    {  
      adv$DCHMOC <- as.numeric(remove.empty.strings(strsplit(adv.lines[1],' ')[[1]]))
      adv.lines <- adv.lines[-1]  
    }
