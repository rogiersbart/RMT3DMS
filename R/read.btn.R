...
#' @export
read.btn <- function(file)
{
  btn.lines <- scan(file, what=character(), sep='\n')
  btn <- NULL
  
  # Data set A1
    btn.lines <- remove.comments.from.lines(btn.lines)
  
  # Data set A2
    btn.lines <- remove.comments.from.lines(btn.lines)
  
  # Data set A3
    dataSetA3 <- remove.empty.strings(strsplit(btn.lines[1],' ')[[1]])
  ...
  # Data set A4
    dataSetA4 <- remove.empty.strings(strsplit(btn.lines[1],' ')[[1]])
    btn.lines <- btn.lines[-1]  
  ...
  # Data set A5
    btn$TRNOP <- as.logical(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
    btn.lines <- btn.lines[-1]  
    
  # Data set A6
    btn$LAYCON <- as.numeric(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
    btn.lines <- btn.lines[-1]
          
  # Data set A7
    dataSetA7 <- get_mt3dms_array(btn.lines,1,btn$NCOL,1)
    btn$DELR <- dataSetA7$mfarray
    btn.lines <- dataSetA7$remaining.lines
    rm(dataSetA7)
          
  # Data set A8
    dataSetA8 <- get_mt3dms_array(btn.lines,1,btn$NROW,1)
    btn$DELC <- dataSetA8$mfarray
    btn.lines <- dataSetA8$remaining.lines
    rm(dataSetA8)
          
  # Data set A9
    dataSetA9 <- get_mt3dms_array(btn.lines,btn$NROW,btn$NCOL,1)
    btn.lines <- dataSetA9$remaining.lines
    btn$HTOP <- dataSetA9$mfarray
    rm(dataSetA9)
          
  # Data set A10
    dataSetA10 <- get_mt3dms_array(btn.lines,btn$NROW,btn$NCOL,btn$NLAY)
    btn.lines <- dataSetA10$remaining.lines
    btn$DZ <- dataSetA10$mfarray
    rm(dataSetA10)
  
  # Data set A11
    dataSetA11 <- get_mt3dms_array(btn.lines,btn$NROW,btn$NCOL,btn$NLAY)
    btn.lines <- dataSetA11$remaining.lines
    btn$PRSITY <- dataSetA11$mfarray
    rm(dataSetA11)
  
  # Data set A12
    dataSetA12 <- get_mt3dms_array(btn.lines,btn$NROW,btn$NCOL,btn$NLAY)
  ...
  # Data set A13
    ...
    {
      dataSetA13 <- get_mt3dms_array(btn.lines,btn$NROW,btn$NCOL,btn$NLAY)
  ...
  # Data set A14
    dataSetA14 <- as.numeric(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
  ...
  # Data set A15
    dataSetA15 <- remove.empty.strings(strsplit(btn.lines[1],' ')[[1]])
  ...
  # Data set A16
    btn$NPRS <- as.numeric(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
    btn.lines <- btn.lines[-1]
  ...
  # Data set A17
    if(btn$NPRS > 0)
    {
      ...
      btn$TIMPRS <- as.numeric(remove.empty.strings(strsplit0(paste(btn.lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]]))
      btn.lines <- btn.lines[-c(1:nLines)]
    }
  
  # Data set A18
    dataSetA18 <- as.numeric(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
  ...
  # Data set A19
    if(btn$NOBS > 0)
    {
      ...
      for(i in 1:btn$NOBS)
      {
        dataSetA19 <- as.numeric(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
  ...
  # Data set A20
    dataSetA20 <- remove.empty.strings(strsplit(btn.lines[1],' ')[[1]])
  ...
  {  
    # Data set A21
      dataSetA21 <- remove.empty.strings(strsplit(btn.lines[1],' ')[[1]])
    ...
    # Data set A22
      if(btn$TSMULT[i] <= 0)
      {
        ...
        btn$TSLNGH[[i]] <- as.numeric(remove.empty.strings(strsplit0(paste(btn.lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]]))
        btn.lines <- btn.lines[-c(1:nLines)]
      }
    
    # Data set A23
      dataSetA23 <- as.numeric(remove.empty.strings(strsplit(btn.lines[1],' ')[[1]]))
