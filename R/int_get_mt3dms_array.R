...
int_get_mt3dms_array <- function(mfarray.lines,NROW,NCOL,NLAY)
{
  ...
  if(prod(dim(mfarray))!=0)
  {
    ...
    { 
      ...
      if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='0') 
      {
        if(NLAY==1)
        {
          mfarray[,,] <- rep(as.numeric(remove_empty_strings(strsplit(mfarray.lines[1],' |\t')[[1]])[2]),NROW*NCOL*NLAY)
          mfarray.lines <- mfarray.lines[-1]
        } else {
          mfarray[,,k] <- matrix(as.numeric(remove_empty_strings(strsplit(mfarray.lines[1],' |\t')[[1]])[2]),nrow=NROW,ncol=NCOL)
          mfarray.lines <- mfarray.lines[-1]
        }
      }
      else if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1] %in% c('100','103'))
      {
        mfarray.lines <- mfarray.lines[-1] 
        nPerLine <- length(as.numeric(remove_empty_strings(strsplit(mfarray.lines[1],' |\t')[[1]])))
        nLines <- (NCOL %/% nPerLine + ifelse((NCOL %% nPerLine)==0, 0, 1))*NROW
        mfarray[,,k] <- matrix(as.numeric(remove_empty_strings(strsplit0(paste(mfarray.lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]])),nrow=NROW,ncol=NCOL,byrow=TRUE)
        mfarray.lines <- mfarray.lines[-c(1:nLines)]
      }
      else if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='EXTERNAL')
      {
        stop('Reading EXTERNAL arrays is not implemented yet...')
      }   
      else if(remove_empty_strings(strsplit(mfarray.lines[1],' ')[[1]])[1]=='OPEN/CLOSE')
      {
  ...
  if(NLAY==1 & (NCOL==1 | NROW==1))
  {
    mfarray <- as.numeric(mfarray)
    class(mfarray) <- 'mt3dms_1d_array'
  }
  if(NLAY==1 & NCOL!=1 & NROW!=1)
  {
    mfarray <- as.matrix(mfarray[,,1])
    class(mfarray) <- 'mt3dms_2d_array'
  }
  if(NLAY!=1) class(mfarray) <- 'mt3dms_3d_array'
