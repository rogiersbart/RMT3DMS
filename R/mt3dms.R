...
rmt_read <- function(file = {cat('Please select nam file ...\n'); file.choose()},
  ...
  if('SSM'%in% ftype) {
    ...
    mt3dms$ssm <- rmt_read_ssm(file = fname[which(mt3dms$nam$ftype == 'SSM')], btn = mt3dms$btn, ftl = fname[which(mt3dms$nam$ftype == 'FTL')], nam = mt3dms$nam, precision = precision)
...
rmt_write <- function(mt3dms, 
  ...
  rmt_write_btn(btn = mt3dms$btn, file = file.path(dir_name, mt3dms$nam$fname[which(mt3dms$nam$ftype == 'BTN')]))
  ...
  if('dsp' %in% ftype) {
    ...
    rmt_write_dsp(dsp = mt3dms$dsp, file = file.path(dir_name, mt3dms$nam$fname[which(mt3dms$nam$ftype == 'DSP')]), btn = mt3dms$btn)
  }

  # ssm
  if('ssm' %in% ftype) {
    if(verbose) print_writing('SSM', file = file.path(dir_name, mt3dms$nam$fname[which(mt3dms$nam$ftype == 'SSM')]))
    rmt_write_ssm(ssm = mt3dms$ssm, file = file.path(dir_name, mt3dms$nam$fname[which(mt3dms$nam$ftype == 'SSM')]), btn = mt3dms$btn)
  }
