...
read_btn <- function(file)
{
  ...
  # Data set A3
    data_set_a3 <- remove_empty_strings(strsplit(btn_lines[1],' ')[[1]])
    btn_lines <- btn_lines[-1]  
    btn$nlay <- as.numeric(data_set_a3[1])
    btn$nrow <- as.numeric(data_set_a3[2])
    btn$ncol <- as.numeric(data_set_a3[3])
    btn$nper <- as.numeric(data_set_a3[4])
    btn$ncomp <- as.numeric(data_set_a3[5])
    btn$mcomp <- as.numeric(data_set_a3[6])
    rm(data_set_a3)
  ...
  # Data set A17
    if(btn$nprs > 0)
    {
      ...
      btn$timprs <- as.numeric(remove_empty_strings(strsplit0(paste(btn_lines[1:nLines],collapse='\n'),' |\t|\n| \n|\n ')[[1]]))
