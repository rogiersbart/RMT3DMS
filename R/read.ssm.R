...
#' @export
read.ssm <- function(file, btn)
{
  ssm.lines <- scan(file, what=character(), sep='\n')
  ssm <- NULL
  
  # Data set D1
    dataSetD1 <- as.logical(remove.empty.strings(strsplit(ssm.lines[1],' ')[[1]]))
  ...
  # Data set D2
    ssm$MXSS <- as.numeric(remove.empty.strings(strsplit(ssm.lines[1],' ')[[1]]))
    ssm.lines <- ssm.lines[-1]  
  ...
  for(stress_period in 1:btn$NPER)
  {
    # Data set D3
      if(ssm$FRCH)
      {
        ssm$INCRCH[stress_period] <- as.numeric(remove.empty.strings(strsplit(ssm.lines[1],' ')[[1]]))
        ssm.lines <- ssm.lines[-1] 
    ...
    # Data set D4
      ...
      {
        dataSetD4 <- get_mt3dms_array(ssm.lines,btn$NROW,btn$NCOL,btn$NCOMP)
    ...
    # Data set D5
      if(ssm$FEVT)
      {
        ssm$INCEVT[stress_period] <- as.numeric(remove.empty.strings(strsplit(ssm.lines[1],' ')[[1]]))
        ssm.lines <- ssm.lines[-1] 
    ...
    # Data set D6
      ...
      {
        dataSetD6 <- get_mt3dms_array(ssm.lines,btn$NROW,btn$NCOL,btn$NCOMP)
    ...
    # Data set D7
      ssm$NSS[stress_period] <- as.numeric(remove.empty.strings(strsplit(ssm.lines[1],' ')[[1]]))
      ssm.lines <- ssm.lines[-1] 
    ...
    # Data set D8
      if(ssm$NSS > 0)
      {
        ...
        {
          dataSetD8 <- as.logical(remove.empty.strings(strsplit(ssm.lines[1],' ')[[1]]))
